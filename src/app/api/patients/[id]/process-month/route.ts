...
/**
 * POST /api/patients/[id]/process-month
 *
 * Body: { year: number, month: number }  (month is 0-based, like JS Date)
 *
 * REGLA DE ORO: el frontend solo envía year+month. El backend lee el texto
 * de sesiones directamente desde Supabase — jamás se envía texto largo.
 *
 * Finds all sessions in the target month with raw_text but no ai_summary,
 * runs generateBatchSessionSummaries (safe: 4-8 sessions = 1 OpenAI call),
 * persists results. Returns: { analyzed: number }
 */
export async function POST(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    // Read sessions directly from Supabase — no text sent from frontend
    const allSessions = await findSessionsByPatient(patientId, user.id)

    const monthSessions = allSessions.filter((s) => {
      if (!s.raw_text?.trim()) return false
      if (s.ai_summary) return false // already analyzed
    ...
    logger.info("process-month complete", {
      patientId,
      year: body.year,
      month: body.month,
      analyzed,
    ...
    logger.error("POST /api/patients/[id]/process-month failed", { error: err.message })
